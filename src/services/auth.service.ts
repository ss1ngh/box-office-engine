...
export const register = async(data : RegisterInput) => {
    ...
    if(existingUser) {
        Logger.warn(`Auth Service : register : Email already exists : ${data.email}`);
...
export const login = async (data : LoginInput) => {
    ...
    if(!user) { 
        Logger.warn(`AuthService : login : User not found : ${data.email}`);
        const error = new Error('User does not exist') as any;
    ...
    if(!isValidPassword) {
        Logger.warn(`AuthService : login : Invalid password for email : ${data.email}`);
